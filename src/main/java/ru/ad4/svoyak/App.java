...
public class App {
    ...
    public void initApplication() {
        if (env.getActiveProfiles().length == 0) {
            ...
            if (activeProfiles.contains(Constants.INSTANCE.getSPRING_PROFILE_DEVELOPMENT())
                    && activeProfiles.contains(Constants.INSTANCE.getSPRING_PROFILE_PRODUCTION())) {
    ...
    private static void addDefaultProfile(@Nonnull final SpringApplication app, @Nonnull final SimpleCommandLinePropertySource source) {
        if (!source.containsProperty("spring.profiles.active")
            ...
            app.setAdditionalProfiles(Constants.INSTANCE.getSPRING_PROFILE_DEVELOPMENT());
        }
    }
}
